class World3 extends Phaser.Scene {
    ...
    preload (){
        ...
        this.load.audio("introduction3", "items/sounds/introduction3.wav")
    };
    ...
    create() {
        ...
        this.intro= this.sound.add("introduction3");
        ...
        gamestate.move={}
        Object.values(gamestate.questions).forEach((elem)=> {
            let q=this.add.image(screenWidth*i,screenHeight*0.95,elem);
            i+=0.2;
            q.setInteractive();
            q.setVisible(false);
            gamestate.questionsButtons.push(q);
            q.displayHeight*=1.05;
            q.displayWidth*=1.05;
...
/*                         gamestate.move[child.name]= [child, q.x, q.y, child.x, child.y, true] //get the question location (the items will move from there)
        ...
        Object.keys(concepts).forEach((elem)=> {
            ...
            img.name= elem;
            img.setInteractive();
            img.setVisible(true); //concept["level"]==1 
